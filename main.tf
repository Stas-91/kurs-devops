...
locals {
  vm_config = {
    ...
    webserv1      = { subnet = yandex_vpc_subnet.subnet-1.id, zone = "ru-central1-a", nat = false, sg = [yandex_vpc_security_group.web_sg.id] }
    webserv2      = { subnet = yandex_vpc_subnet.subnet-2.id, zone = "ru-central1-b", nat = false, sg = [yandex_vpc_security_group.web_sg.id] }
...
resource "yandex_compute_instance" "vm" {
  ...
  metadata = {
    user-data = file("./meta/user-data.yaml")
  }
