...
- Что не удалось автомотизировать:
  - Я решил получать сертификат SSL для сайта через Яндекс Let's Encrypt используя DNS_TXT чтобы это подходило для ALB, но выдача такого сертификата занимает рандомное время до 24 часов, поэтому сертификат я выпустил из Консоли Управления Yandex Cloud
  - Также я хотел полностью исключить необходимось ручной правки конфигурации, поэтому постарался убрать имя пользователя в переменную, но все же осталась необхомость задавать ее в ansible.cfg и inventory.ini
