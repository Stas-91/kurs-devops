...
- Изменения Сруктуру проекта терраформ:
    ...
    - Изменено network.tf струкутра сети - теперь для каждой ВМ в истнансе и PostпкуSQL отдельная подсеть (websub1,2,3), добавлена А запись в ДНС (связывает доменное имя с IP-адресом балансировщика)
    - Изменено security_groups.tf - указаны новые подсети и ресурсы
    - Изменено alb.tf - балансировщик перенастроен на hhtps и группу ВМ в качестве целевой группы
    - Также небольшие изменения произведены в main.tf, variables.tf, terraform.tfvars и ansible.tf. Об этом будет сказано ниже (Основные этапы работы проекта terraform)
    - Из каталога meta теперь используются templatefile передавая имя пользователя через переменную (также где это было возможно имя пользователя тоже передается через переменную var.user_vm)
...
- Отдельным пунктом необходимо рассказать о реализации дополнительных задач со стороны Ansible
  - Всвязи с использование instance group, Prometheus уже нельзя было передать IP адреса хостов созданных ВМ для вебсерверов, т.к. хосты в instance group могут меняться. Для этого в конфигурации прометеуса применен  job dynamic_targets. Создается targets.json с помощью скрипта scan_exporters.sh, который опрашивает нужные подсети и записывает их в targets.json.
